...
<div <?php wc_product_class('col-lg-'.esc_attr($column).' col-sm-6'); ?>>
    <div class="sale_product_item pr_grid">
        <?php if($product->is_on_sale()) : ?>
            <div class="ribbon"><span class="text"> <?php esc_html_e('Sale', 'appland') ?> </span></div>
        <?php endif; ?>
        <?php if ( ! $product->managing_stock() && ! $product->is_in_stock() ) : ?>
            <div class="ribbon"><span class="text"> <?php esc_html_e('Stock Out', 'appland') ?> </span></div>
        <?php endif; ?>
        <div class="product">
            ...
            <div class="hover_contents">
                ...
                <?php if(shortcode_exists('ti_wishlists_addtowishlist')): ?>
                    <span class="wihslist-btn" title="<?php esc_attr_e('Add To Wishlist', 'appland') ?>">
                ...
                <!--<span class="like" title="<?php /*esc_attr_e('Compare', 'appland') */?>">
                    <a href=""><i class="ti-tag"></i></a>
                </span>-->
            </div>
            <?php

            $title_align = !empty($opt['product_title_align']) ? $opt['product_title_align'] : '';
            $title_align = $title_align == 'center' ? 'center_product' : '';
            ?>
            <div class="pr_details <?php echo esc_attr($title_align) ?>">
